...
void Resolver::Run(const std::string& domain, const std::string& type_str) {
    ...
    std::string current_ns_ip = root_servers[0];
    ...
    while (!solved) {
        ...
        if (response.empty()) {
            std::cerr << "No response or error." << std::endl;
            return;
        }
        ...
        if (found_answer) {
            ...
            if (!glue_found) {
                 ...
                 std::cerr << "No glue record found for " << referrals[0] << ", giving up." << std::endl;
                 return;
            }
        } else {
...
std::vector<DnsRecord> Resolver::QueryServer(const std::string& ip, const std::string& domain, uint16_t type) {
    DnsHeader header = {};
    header.id = htons(GetCurrentProcessId());
...
std::vector<DnsRecord> Resolver::SendUdp(const std::string& ip, const std::vector<uint8_t>& packet) {
    ...
    sendto(sock, (const char*)packet.data(), packet.size(), 0, (sockaddr*)&srv, sizeof(srv));

    std::vector<uint8_t> buf(4096);
    sockaddr_in from;
    int fromlen = sizeof(from);
    int len = recvfrom(sock, (char*)buf.data(), buf.size(), 0, (sockaddr*)&from, &fromlen);
    ...
    if (len > 0) {
        ...
        if (ntohs(h->flags) & 0x0200) {
            if (debug_mode) std::cout << "TC flag set, switching to TCP." << std::endl;
            return SendTcp(ip, packet);
        }
        return ParseResponse(buf, packet.size());
    }
    return {};
}
...
std::vector<DnsRecord> Resolver::SendTcp(const std::string& ip, const std::vector<uint8_t>& packet) {
    ...
    if (connect(sock, (sockaddr*)&srv, sizeof(srv)) < 0) return {};
...
std::vector<DnsRecord> Resolver::ParseResponse(const std::vector<uint8_t>& response, int query_len) {
    ...
    for(int i=0; i<q_count; ++i) {
        ParseName(response.data(), pos, response.size());
        pos += 4;
    }
    ...
    for (int i = 0; i < total_rr; ++i) {
        ...
        DnsRecord rec;
        rec.name = ParseName(response.data(), pos, response.size());
        ...
        if (rec.type == A) {
            ...
            int tmp = pos;
            rec.parsed_data = ParseName(response.data(), tmp, response.size());
        }
